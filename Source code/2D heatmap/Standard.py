...
def normalization(file,Cbarsize,clable,Clablesize,dpi,cblock,y_font,x_font,Class,standard_scale,data,datasize,cmap,vmin,vmax,y,x,cbar,cbarscale,linewidths,linecolor,square,row_cluster,col_cluster,ylabelsize,xlabelsize,y_rotation,x_rotation,ycolor,xcolor,metric,method,treestyle,treecolor,treewidths,robust):
    ...
    with open(file, 'r') as csvfile:
        ...
        for row in plots:
            yz.append(row[0])  # 从csv读取的数据是str类型
            # print("y:",y)
            ycount = len(yz)
        print(ycount)
        while "" in header_row:
    ...
    g = sns.clustermap(df,robust=robusts,
                   ...
                   metric = metrics,
                   method = methods,
                   standard_scale=standard_scales,
